...
def process_video(input, output,z):
    ...
    width_a = clip.w - 160
    height_a = clip.h - 10
    print(clip.w,clip.h, clip.fps,duration01,duration02,duration03,duration04,duration05,duration06,duration07, duration_main,z,input)
    ...
    a7 = (input_stream.audio.filter('atrim', start=duration07, end=duration_main)
          .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.01).filter('volume',1.05))  # Khúc 8/Zoom 105% , Xoay 5deg, Crop-Top 110px,Crop-Bottom-110px Contrast=0.89, Brightness=0.028,saturation=1.05,Audio Speed:1.05
    ...
    subprocess.call(["ffmpeg","-i",input,"-i", z, "-map","1","-map","0","-c","copy","-disposition:0","attached_pic", output,])
...
def get_video_paths(folder_path):
    ...
    for z in file_srt:
        ...
        if z == ".DS_Store":
            ...
            path_srt_list.append(srt_path + z)
