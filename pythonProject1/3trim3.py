...
def process_video(input, output):
    ...
    v0 = (input_stream.video.filter('trim', start=duration04, end=duration05)
    ...
    a0 = (input_stream.audio.filter('atrim', start=duration04, end=duration05)
          .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.03).filter('volume', 0.85))
    
    v1 = (input_stream.video.filter('trim', start=duration05, end=duration_main)
    ...
    a1 = (input_stream.audio.filter('atrim', start=duration05, end=duration_main)
...
def get_video_paths(folder_path):
    ...
    for name in file_name_list:
        ...
        if name == ".DS_Store":
            ...
            final_name_list.append(folder_path + "part 3_3_" + name)
