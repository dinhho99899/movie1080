...
def process_video(input, output):
    ...
    a4 = (input_stream.audio.filter('atrim', start=duration04, end=duration05)
         .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.02).filter('volume', 1.03))
    ...
    a5 = (input_stream.audio.filter('atrim', start=duration05, end=duration06)
          .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.01).filter('volume', 1.01))
    ...
    a6 = (input_stream.audio.filter('atrim', start=duration06, end=duration07)
          .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.02).filter('volume', 0.96))
    ...
    a7 = (input_stream.audio.filter('atrim', start=duration07, end=duration_main)
          .filter('asetpts', expr='PTS-STARTPTS').filter('atempo', 1.01).filter('volume',0.9))  # Khúc 8/Zoom 105% , Xoay 5deg, Crop-Top 110px,Crop-Bottom-110px Contrast=0.89, Brightness=0.028,saturation=1.05,Audio Speed:1.05
    joined = ffmpeg.concat(v0, a0, v1, a1, v2, a2, v3, a3, v4, a4, v5, a5, v6, a6, v7, a7, v=1, a=1).node
    v8 = joined[0].filter('crop',width_a, height_a).filter('lut', a=155 * 0.5).filter('zoompan',z=f'if(lte(mod(it*30,180),30),min(max(zoom,pzoom)+0.008,1.5),min(max(zoom,pzoom)-0.008,1.5))', x='iw/2-(iw/zoom/2)', y='ih/2-(ih/zoom/2)', d=1,s=f'{width_a}x{height_a}', fps=30)
    a8 = joined[1]
